...
(async () => {
  ...
  console.log(' Extracted summary data. Now checking medications...');
  ...
  for (let i = 0; i < extractedData.length; i++) {
    ...
    try {
      ...
      if (noMedsText) {
        console.log(`No medications recorded for ${extractedData[i].name}`);
      ...
      // I think this should not be here, but I don't know where to put it
      await page.waitForSelector('div.oe-dropdown-toggle');
      await page.click('div.oe-dropdown-toggle');
      await page.waitForSelector('.menuEntries li');
      const items = await page.$$('.menuEntries li');
...
// Não tem resultado porque o código quebra antes de chegar aqui
